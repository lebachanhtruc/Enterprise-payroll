...
export default function ImportData({ employees, timesheets, settings, pastData, setPastData, setEmployees, setSettings, setTimesheets, setActiveTab }: ImportDataProps) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in">
            <div className="bg-white p-10 rounded-xl shadow-sm border border-slate-200 flex flex-col items-center justify-center text-center max-w-2xl mx-auto">
                ...
                <input type="file" id="json-upload" accept=".json" className="hidden" onChange={handleImportMenu} />
                <div className="flex flex-col gap-3 w-full max-w-md">
                    <button title="Import previous payroll JSON data to populate the current cycle" 
                        onClick={() => document.getElementById('json-upload')?.click()} 
                        className="w-full bg-indigo-600 text-white font-bold px-6 py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-indigo-700 transition-colors text-lg shadow-sm"
                    >
                ...
                {pastData && (
                    <button title="Clear all current initialized data and start fresh" 
                        onClick={handleDeleteImport} 
                        className="w-full max-w-md mt-4 bg-rose-50 text-rose-600 font-bold px-6 py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-rose-100 transition-colors"
                    >
                        ...
                        <table className="w-full text-left min-w-[850px] table-fixed">
                            <thead>
                                <tr className="bg-slate-800 text-white">
                                    <th className="px-5 py-3 w-48 sticky left-0 z-20 bg-slate-800 border-r border-slate-700 shadow-[2px_0_5px_rgba(0,0,0,0.15)]" rowSpan={2}>CTV</th>
