...
export default function TimesheetInput({ employees, timesheets, onTimeChange, setTimesheets, settings, setAnomalies, setShowAnomaliesModal, setViewLogsConfig, hasUnsavedChanges, setHasUnsavedChanges, isLoadingEmployees }: TimesheetInputProps) {
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto items-center">
          ...
          <div className="flex gap-2 w-full sm:w-auto justify-end">
            ...
            <button 
              ...
              title={hasValidationErrors ? "Fix validation errors before saving" : "Temporarily save changes to session memory"}
            >
              {hasValidationErrors ? 'Invalid Entries' : 'Save Changes'}
            </button>

            <button onClick={() => fileInputRef.current?.click()} className="text-white bg-indigo-600 px-4 py-2 border border-indigo-600 rounded-lg text-sm font-bold hover:bg-indigo-700 shadow-sm transition-colors cursor-pointer whitespace-nowrap" title="Import CSV files containing timesheet data directly from the Point of Sale system">Sync from POS</button>
            <button onClick={handleResetFormClick} className="text-slate-600 bg-white px-4 py-2 border border-slate-300 rounded-lg text-sm font-bold hover:bg-slate-50 transition-colors shadow-sm cursor-pointer whitespace-nowrap" title="Clear all current timesheet inputs and start over">Reset Form</button>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {isLoadingEmployees && employees.length === 0 ? (
              ...
              paginatedEmployees.map((emp: Employee) => {
                ...
                const renderInput = (field: string, bg: string, locked: boolean) => {
                    ...
                    return (
                        <input 
                            ...
                            className={cn(
                                ...
                                hasError && "border-rose-500 bg-rose-50 text-rose-700 animate-pulse focus:border-rose-600"
                            )} 
                        />
                    );
                };
                    ...
                    <td className="px-6 py-4 border-r border-slate-200 sticky left-0 z-10 bg-white drop-shadow-md shadow-[2px_0_5px_rgba(0,0,0,0.05)]">
                      ...
                      {isFixed && (
                          <div className="mt-1 flex items-center gap-2">
                            ...
                            <button onClick={() => toggleUnlock(emp.id, emp.rule)} className="text-slate-400 hover:text-slate-700" title="Toggle edit lock for this employee">{isUnlocked ? <Lock size={14} /> : <Unlock size={14}/>}</button>
                          </div>
                      )}
