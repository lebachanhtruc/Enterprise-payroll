...
export default function DevTestRunner({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const [testResults, setTestResults] = useState<Record<string, { passed: boolean; assertions: any[]; results: any[] }> | null>(null);
  ...
  const handleRunTests = () => {
    setIsRunning(true);
    setTimeout(() => {
      const results: Record<string, { passed: boolean; assertions: any[]; results: any[] }> = {};
  ...
  const passRate = testResults ? Math.round((passedTests / totalTests) * 100) : 0;
                                  ...
                                  {result.assertions.map((assertion, index) => (
                                    <div key={index} className="flex items-start justify-between gap-4 p-2.5 rounded-lg bg-slate-900/40 border border-slate-850">
                                      ...
                                      <div className="text-right shrink-0 font-mono text-[10px] space-y-0.5">
                                        <div className="text-slate-400">Actual: <span className={assertion.passed ? 'text-emerald-400' : 'text-rose-400'}>{JSON.stringify(assertion.actual)}</span></div>
