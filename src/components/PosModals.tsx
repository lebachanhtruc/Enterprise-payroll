...
const PosLogEditRow = ({ log, logIdx, empId, onConfirm, onValidate, auditLog, isLocked }: PosLogEditRowProps) => {
            ...
            <td className="p-4 text-right w-48">
                {editMode ? (
                    ...
                    <div className="flex gap-2 justify-end items-center">
                        {hasSessionAnomaly(log.sessions) ? (
                            !log.isValidated ? (
                                confirmValidate ? (
                                    ...
                                    <button disabled={isLocked} onClick={() => setConfirmValidate(true)} className={`px-3 py-1 border rounded font-bold text-xs shadow-sm ${isLocked ? 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border-emerald-200'}`} title="Mark this anomaly as validated and correct">Validated</button>
                                )
                            ) : (
                                auditLog ? (
                        ...
                        <button disabled={isLocked} onClick={() => setEditMode(true)} className={`px-3 py-1 border rounded font-bold text-xs shadow-sm ${isLocked ? 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 border-slate-200'}`}>Edit</button>
                    </div>
                )}
            </td>
        </tr>
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody>
                                    {(timesheets[viewLogsConfig.empId]?.posLogs || [])
                                        ...
                                        .map(({ log, idx }: any) => (
                                        <PosLogEditRow key={idx} log={log} logIdx={idx} empId={viewLogsConfig.empId!} onConfirm={handlePosLogEdit} onValidate={handlePosLogValidate} auditLog={auditLogs[`${viewLogsConfig.empId}_${log.date}`]} isLocked={isLocked} />
                                    ))}
                                </tbody>
