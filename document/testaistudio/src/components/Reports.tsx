...
export default function Reports({ results, settings, employees, timesheets, isLoadingEmployees }: { results: PayrollResult[], settings: SystemSettings, employees: Employee[], timesheets: Record<number, Timesheet>, isLoadingEmployees?: boolean }) {
    ...
    const executePayrollSave = async () => {
        ...
        try {
            ...
            // Backup JSON download (Thực hiện trước để đảm bảo luôn tải được file kể cả khi DB lỗi)
            try {
                ...
                // Fallback: Tự động tải xuống mặc định
                const data = { employees, timesheets, settings, payrollData: results };
                const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
            ...
            const successMsg = 'Success! Payroll confirmed, saved to Supabase, and backup downloaded.';
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col 2xl:flex-row justify-between items-start 2xl:items-center gap-6 bg-white p-6 rounded-xl shadow-sm border border-slate-200 print-hidden">
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <button title="Finalize payroll and save to history" 
                    ...
                    <button 
                        ...
                        className="bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-emerald-700 transition-all whitespace-nowrap" 
                        title="Export raw data to Excel for accounting"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="8" y1="13" x2="16" y2="13"></line><line x1="8" y1="17" x2="16" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                        Export Excel
                    </button>
                    <button onClick={handlePrint} className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-indigo-700 transition-all whitespace-nowrap" title="Print the current report view"><Printer size={18} /> Print Report</button>
                </div>
            </div>
